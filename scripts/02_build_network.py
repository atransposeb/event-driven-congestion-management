...
def build_network(output_path: Path, place: str, force: bool = False) -> None:
    try:
        import osmnx as ox
        ...
        if output_path.exists() and output_path.stat().st_size > 1_000_000 and not force:
...
def main() -> None:
    ...
    parser.add_argument("--force", action="store_true", help="Replace any existing graph if OSMnx is unavailable.")
    args = parser.parse_args()
    ensure_directories()
    build_network(args.output, args.place, force=args.force)
