...
def optimize(prediction_path: Path, graph_path: Path, output_path: Path, officers: int) -> dict[str, Any]:
    ...
    LOGGER.info("Scoring %s candidate intersections for %s available officers", len(candidates), officers)
    ...
    if cp_model is None:
        ...
        return payload
    LOGGER.info("Solving CP-SAT manpower allocation with %s deployable officers", deployable)
...
def _load_cp_sat() -> Any | None:
    LOGGER.info("Loading OR-Tools CP-SAT optimizer")
    try:
